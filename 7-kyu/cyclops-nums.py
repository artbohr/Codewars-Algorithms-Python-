def cyclops (n):
    s = bin(n)[2:]
    if not s.count("1")==len(s)-1 or len(s)%2==0 or not s[len(s)//2]=="0":
        return False
    return True

'''
A cyclops number is a number in binary that is made up of all 1's, with
 one 0 in the exact middle. That means all cyclops numbers must have
 an odd number of digits for there to be an exact middle.
A couple examples:
101
11111111011111111
