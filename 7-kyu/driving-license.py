def driver(data):
    ...
    name = data[2]
    name = data[2][:5].upper() if len(data[2])>4 else name.upper()+(5-len(name))*"9"
    decade = data[3][-2]
    month = months[data[3][3:6]]
    month = month if data[4]=="M" else str(int(month[0])+5) + month[1]
    day = data[3][:2]
    year = data[3][-1]
    initial1 = data[0][0]
    initial2 = data[1][0] if data[1] else "9"

    return "{}{}{}{}{}{}{}9AA".format(name,decade,month,day,year,initial1,initial2)
...
Introduction
     ...
     vehicle on any highway or other road defined in s.192 Road Traffic Act 1988[1]
     irrespective of ownership of the land over which the road passes, thus including many
     which allow the public to pass over private land; similar requirements apply in Northern
     Ireland under the Road Traffic (Northern Ireland) Order 1981. (Source Wikipedia)
