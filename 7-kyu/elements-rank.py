def rank_of_element(arr,i):
    ...
    for c,x in enumerate(arr):
        if c<i and arr[i] - x >= 0:
            score +=1
        elif c==i:
            continue
        elif c>i and arr[i] - x > 0:
            score +=1
...
The rank of the arr[i] is a value equal to the number of elements less
than or equal to arr[i] standing before arr[i], plus the number of elements
less than arr[i] standing after arr[i].
