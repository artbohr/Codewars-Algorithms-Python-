def quicksum(packet):
    if not packet.replace(' ','').isalpha() or packet.upper()!=packet: return 0
    return sum((i+1) * (ord(x)-64) for i,x in enumerate(packet) if x!=' ')
...
A checksum is an algorithm that scans a packet of data and returns a single number.
 ...
 so checksums are often used for detecting transmission errors, validating
...
When the packet doesn't have only uppercase letters and spaces or just spaces the
 result to quicksum have to be zero (0).
