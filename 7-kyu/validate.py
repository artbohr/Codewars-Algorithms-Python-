...
def validate(message):
    m = message.split()
    lp = ''.join(m[4:])
    return m[0]=="MDZHB" and len(m[1])==2 and m[2].isdigit() and len(m[2])==3 and m[3].isalpha() and len(lp)==8 and lp.isdigit()
...
(see also https://en.wikipedia.org/wiki/UVB-76). Most of time specific "buzz"
 ...
 interrupted and a voice transmission in Russian takes place. Transmitted messages
 have always the same format like this:
