def balanced_num(number):
    ...
    n_str, mid = str(number), len(str(number))//2
    b = sum(int(n) for n in n_str[:mid-1 if len(n_str)%2==0 else mid])==sum(int(n) for n in n_str[mid+1:])
    return "Balanced" if b else "Not Balanced"
