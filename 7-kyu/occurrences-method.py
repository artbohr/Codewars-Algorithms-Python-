class List():
    def count_spec_digits(self, integers_list, digits_list):
        f_list = "".join(str(abs(x)) for x in integers_list)
        return [(x, f_list.count(str(x))) for x in digits_list]
...
We need a method in the List Class that may count specific digits from a given
 ...
 that has the specific digits to count which length cannot be be longer
 ...
 its corresponding total frequency in all the integers of the first list. This list of
 tuples should be ordered with the same order that the digits have in digitsList
