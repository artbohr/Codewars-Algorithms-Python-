...
def mean(town, strng):
    for x in strng.split("\n"):
        if town in x and ':'==x[len(town):len(town)+1]:
            nums = [float(i) for i in re.findall("\d+\.\d+", x)]
...
def variance(town, strng):
    for x in strng.split("\n"):
        if town in x and ":"==x[len(town):len(town)+1]:
            nums_sqr = [(float(i)-mean(town, strng))**2 for i in re.findall("\d+\.\d+", x)]
            return sum(nums_sqr)/len(nums_sqr)
    return -1
...
mean("Beijing", data)
variance("Beijing", data)
# Returns for mean: 52.416666666666664
# Returns for variance: 4808.37138888889
