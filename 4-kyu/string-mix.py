...
def mix(s1, s2):
    ...
    str1 = Counter(s1)
    str2 = Counter(s2)
    uni = (str1|str2).most_common()
    clean = []


    for ele in uni:
        if ele[0].isalpha() and ele[1] > 1 and ele[0].islower():
            if ele in str1.items() and ele not in str2.items():
                clean.append((ele[0], ele[1], 3))
            elif ele in str2.items() and ele not in str1.items():
                clean.append((ele[0], ele[1], 2))
            else:
                clean.append((ele[0], ele[1], 1))


    for i in sorted(clean, key=lambda x: (-x[1], -x[2], x[0])):
        output += '{}:{}/'.format(['=','2','1'][i[2]-1], i[0] * i[1])
